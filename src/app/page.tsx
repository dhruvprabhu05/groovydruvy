...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ sector?: string; type?: string }> }) {
  const params = await searchParams;
  const [summary, articles, recommended, watchlistStocks, earnings] = await Promise.all([
    getLatestMarketSummary(), getArticles(params.sector, params.type), getRecommendedStocks(), getWatchlistStocks(), getUpcomingEarnings(),
  ]);
  ...
  return (
    <main>
      ...
      {lastUpdated && <div style={{ padding: "0 24px", fontSize: "11px", color: "var(--text-muted)" }}>Last updated: {lastUpdated}</div>}
